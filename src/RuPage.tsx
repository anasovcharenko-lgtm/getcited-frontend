...
function Modal({ onClose, onAuditComplete }: { onClose: () => void; onAuditComplete: (data: AuditData) => void }) {
  ...
  const startAudit = async (e: React.FormEvent) => {
    ...
    if (mode === "manual" && promptsToRun.length === 0) { setError("Добавьте хотя бы один промпт или переключитесь на генерацию."); return; }
    setLoading(true);
    setError("");
    try {
      const { data: { user } } = await supabase.auth.getUser();
      ...
      if (!res.ok) {
        ...
        setError(detail ? `Не удалось запустить аудит: ${detail}` : "Что-то пошло не так. Попробуйте ещё раз.");
        setLoading(false);
        return;
      }
